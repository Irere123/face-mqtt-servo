...
# ESP32 MicroPython port. If umqtt.simple is missing, install it once with:
...
# Servo Configuration
# SG90 Servo: 50Hz PWM. 10-bit duty 40-115 ~ 0.8-2.25ms pulse ~ 0-180 deg.
# Any free GPIO works (13, 14, 25, 26, 27...). GPIO 18 avoids the ESP32
# strapping/JTAG pins, so the servo stays still during boot.
SERVO_PIN = 18
    ...
    try:
        ...
        while True:
            ...
            if time.time() - last_heartbeat > 10:
                payload = ujson.dumps(
                    {"node": "esp32", "status": "ONLINE", "uptime": time.time()}
                )
